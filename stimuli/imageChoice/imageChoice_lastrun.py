...
expName = 'imageChoice'  # from the Builder filename that created this script
expInfo = {'姓名': '', '性别': '', '年龄': ''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\const\\OneDrive\\Desktop\\research\\psychopyProj\\1_mouse-egi\\word-image\\imageChoice\\imageChoice_lastrun.py',
...
image_item_2 = visual.ImageStim(
    win=win,
    name='image_item_2', units='norm', 
    image='sin', mask=None,
    ori=0, pos=(0, 0), size=None,
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
...
images = data.TrialHandler(nReps=1, method='random', 
...
for thisImage in images:
    ...
    mouse.clicked_name = []
    gotValidClick = False  # until a click is received
    image_item.setImage(item)
    op_left.setText(option_left)
    op_right.setText(option_right)
    # keep track of which components have finished
    imageComponents = [mouse, image_item, rect_left, rect_right, op_left, op_right]
    ...
    while continueRoutine:
        ...
        # *image_item* updates
        if image_item.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
    ...
    images.addData('image_item.started', image_item.tStartRefresh)
    images.addData('image_item.stopped', image_item.tStopRefresh)
    images.addData('op_left.started', op_left.tStartRefresh)
    images.addData('op_left.stopped', op_left.tStopRefresh)
    ...
    image_item_2.setImage(item)
    op_left_2.setText(option_left)
    op_right_2.setText(option_right)
    # keep track of which components have finished
    displayComponents = [image_item_2, rect_left_2, rect_right_2, op_left_2, op_right_2]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *image_item_2* updates
        if image_item_2.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
            image_item_2.frameNStart = frameN  # exact frame index
            image_item_2.tStart = t  # local t and not account for scr refresh
            image_item_2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(image_item_2, 'tStartRefresh')  # time at next scr refresh
            image_item_2.setAutoDraw(True)
        if image_item_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > image_item_2.tStartRefresh + 1-frameTolerance:
                # keep track of stop time/frame for later
                image_item_2.tStop = t  # not accounting for scr refresh
                image_item_2.frameNStop = frameN  # exact frame index
                win.timeOnFlip(image_item_2, 'tStopRefresh')  # time at next scr refresh
                image_item_2.setAutoDraw(False)
    ...
    images.addData('image_item_2.started', image_item_2.tStartRefresh)
    images.addData('image_item_2.stopped', image_item_2.tStopRefresh)
    images.addData('op_left_2.started', op_left_2.tStartRefresh)
    images.addData('op_left_2.stopped', op_left_2.tStopRefresh)
