...
class mouseData:
    ...
    def time_normalize(self):
        ...
        self.data.nx = self.data.nx.apply(np.array)
        self.data.ny = self.data.ny.apply(np.array)

    def time_extend(self):
        # max_time = 5000
        self.data['rx'] = [squeak.uniform_time(x, t, max_duration=max(t)) for x, t in zip(self.data.x, self.data.t)]
        self.data['ry'] = [squeak.uniform_time(x, t, max_duration=max(t)) for x, t in zip(self.data.x, self.data.t)]
    ...
    # def plot_avg_trajectory(self, condition, condition0, condition1, title="Average trajectories"):
    ...
    # def plot_avg_trajectory_and_normal(self, condition, condition0, condition1, sample_frac=1):
    #     condition0_indices = self.data[self.data[condition] == condition0].index
    #     condition1_indices = self.data[self.data[condition] == condition1].index
    #     nx = np.array(list(self.data.nx)).T
    #     ny = np.array(list(self.data.ny)).T
